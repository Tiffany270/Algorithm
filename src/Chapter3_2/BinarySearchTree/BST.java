...
public class BST<Key extends Comparable<Key>, Value> {
	...
	/*
	 * 二叉查找树 1.能将链表插入的灵活性和有序数组查找的高效性结合起来 数据结构由结点组成 --》每个结点只有一个父结点（根节点除外），
	 * 每个结点都只有左右两个链接，分别指向自己的左子节点和右子节点。 每个链接看作指向另一颗二叉树 结点包含了一个键和值，键之间也有顺序之分以支持高效的查找
	 * 2.每个节点都含有一个Comparable的键 且每个结点的键都大于其左子树的任意结点的键， 而小于右子树的任意结点的键 3.查找和插入都运用了递归算法
	 */
	...
	/*
	 * 最小键：如果左链接为空，一颗二叉查找树中最小的键就是根节点 如果左链接不为空，那么树中最小键就是左子树中的最小键 最大键同理
	 */
	public Key min() {
		return min(root).key;
	}
	...
	/*
	 * 删除最小键 不断深入根结点的左子树中直到遇到一个空链接，然后将该结点的链接指向该结点的右子树 而没有任何链接指向的结点会被删除并被垃圾收集器清理掉
	 */
